...
void FramePublisher::DrawTextInfo(cv::Mat &im, int nState, cv::Mat &imText)
{
    ...
    if(nState==Tracking::NO_IMAGES_YET)
        ...
        s << " NOT INITIALIZED "<< " , Features: "<< mvpMatchedMapPoints.size();
    else if(nState==Tracking::INITIALIZING)
        s << " TRYING TO INITIALIZE "<< " , Features: "<< mvpMatchedMapPoints.size();
    else if(nState==Tracking::WORKING)
    {
        s << " TRACKING ";
        int nKFs = mpMap->KeyFramesInMap();
        int nMPs = mpMap->MapPointsInMap();
        s << " - KFs: " << nKFs << " , MPs: " << nMPs << " , Tracked: " << mnTracked << " , Features: "<< mvpMatchedMapPoints.size();
    }
    else if(nState==Tracking::LOST)
    {
        s << " TRACK LOST. TRYING TO RELOCALIZE "<< " , Features: "<< mvpMatchedMapPoints.size();
    }
